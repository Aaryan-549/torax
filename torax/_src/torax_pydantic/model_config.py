...
from torax._src import physics_models, version
...
class ToraxConfig(torax_pydantic.BaseModelFrozen):
    ...
    def _v1_compatibility(cls, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if (
            "numerics" in configurable_data
            and "calcphibdot" in configurable_data["numerics"]
        ):
    ...
    def _check_psidot_and_evolve_current(self) -> typing_extensions.Self:
        ...
        if self.profile_conditions.psidot is not None and self.numerics.evolve_current:
            logging.warning("""
